...
module Semrush
  ...
  API_ANALYTIC_URL = "http://api.semrush.com/analytics/v1/?type=%TYPE%&target_type=%REQUEST_TYPE%&target=%REQUEST%&key=%API_KEY%&display_limit=%LIMIT%&display_offset=%OFFSET%&export=api&database=%DB%&export_columns=%EXPORT_COLUMNS%&display_sort=%DISPLAY_SORT%&display_filter=%DISPLAY_FILTER%&display_date=%DISPLAY_DATE%"
